...
public class IMDBDirectorLinker extends BaseLinker implements Linker {

	private static final Logger log = LoggerFactory.getLogger(IMDBDirectorLinker.class) ;
	
	public IMDBDirectorLinker(String base) {
		super(base);
	}
	...
	public String getURL(String name) {
    	...
    	try {
        	...
        	if ( document.title().startsWith("IMDb Name Search") ) {
        		// TODO: this does not seem to work: "p:has(b:matches(Exact Matches)) small:matches(\\(Director,)"
            	Elements elements = document.select("b:matches(Exact Matches)");
            	if ( elements.size() == 1 ) {
            		Element parent = elements.first().parent();
            		Elements elements2 = parent.select("small:matches(\\(Director,)");
            		if ( elements2.size() == 1) {
